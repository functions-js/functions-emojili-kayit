    ...
    run: async(client, message, args) => {
        ...
        collector.on("collect", async(reaction, user) => {
            ...
            if (reaction.emoji.id == '788870665645719582') { //erkek
                ...
                msg.edit(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setDescription(`**${member}** adlı kullanıcı \`${name} | ${age}\` olarak kayıt edildi.(<@&789476656061743124>) \n\n Kullanıcının Son İsimleri: \n ${isimler}`).setColor('#2e3033').setFooter(`'.isimler @kullanıcı' yaparak kullanıcının eski isimlerine bakabilirsiniz.`))
            }
            if (reaction.emoji.id == '788870666925113414') { //kız
                ...
                return msg.edit(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setDescription(`**${member}** adlı kullanıcı \`${name} | ${age}\` olarak kayıt edildi.(<@&789476654967160854>)\n\n Kullanıcının Son İsimleri \n ${isimler}`).setColor('#2e3033').setFooter(`'.isimler @kullanıcı' yaparak kullanıcının eski isimlerine bakabilirsiniz.`))
            }
