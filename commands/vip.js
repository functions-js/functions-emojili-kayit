...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        await msg.react('a:oceanus_yesil:788870665645719582') // erkek emojileri soldaki gibi giriniz
        await msg.react('a:oceanus_mavi:788870666925113414') //kız
        ...
        collector.on("collect", async(reaction, user) => {
            ...
            if (reaction.emoji.id == '788870665645719582') { //erkek
                ...
                msg.edit(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setDescription(`**${member}** adlı kullanıcı \`${name} | ${age}\` olarak **VİP** kayıt edildi.(<@&789476656061743124>) \n\n Kullanıcının Son İsimleri: \n ${isimler}`).setColor('#2e3033').setFooter(`'.isimler @kullanıcı' yaparak kullanıcının eski isimlerine bakabilirsiniz.`))
            }
            if (reaction.emoji.id == '788870666925113414') { //kız
                ...
                await message.guild.members.cache.get(member.id).roles.add(client.config.vipRoles)
                return msg.edit(new Discord.MessageEmbed().setAuthor(message.member.displayName, message.author.avatarURL({ dynamic: true })).setDescription(`**${member}** adlı kullanıcı \`${name} | ${age}\` olarak **VİP** kayıt edildi.(<@&789476654967160854>)\n\n Kullanıcının Son İsimleri \n ${isimler}`).setColor('#2e3033').setFooter(`'.isimler @kullanıcı' yaparak kullanıcının eski isimlerine bakabilirsiniz.`))
            }
